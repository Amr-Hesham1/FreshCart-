...
import checkOut from '../../assets/Ecommerce checkout laptop-rafiki.png'
...
export default function CheckOut() {
    ...
    async function request(shippingAddress) {
        try {
            ...
            let { data } = await axios.post(`https://ecommerce.routemisr.com/api/v1/orders/checkout-session/${Cart.cartId}?url=${window.location.protocol}//${window.location.hostname}:${window.location.port}`, {shippingAddress} , {headers})
            console.log(data);
            setIsVisible(true);
            setIsLoading(false);
    ...
    return (
        <>
            <div className='my-10 m-auto lg:gap-10 grid md:grid-cols-5'>
                ...
                <img src={checkOut} className='md:col-span-2 w-full ' alt="CheckOut" />
                <form onSubmit={Formik.handleSubmit} className=" w-4/5 m-auto md:col-span-3">
                    <h1 className='text-center font-extrabold text-5xl text-sky-700 italic font-serif py-9 lg:py-16'>Check Out      <i className="fa-solid fa-credit-card"></i></h1>
